...
const SuratUnit = () => {
  ...
  const fetchSurat = async () => {
    try {
      ...
      // 🔥 LOGIKA UNIT: Ambil data surat yang relevan
      // Menggunakan optional chaining (?.role) untuk mencegah error 'null'
  ...
  if (loading) {
    return (
      <UnitLayout title="Daftar Surat Masuk">
  ...
  return (
    <UnitLayout title="Daftar Surat Masuk">
      {/* --- TOOLBAR: SEARCH (GAYA DASHBOARD ADMIN) --- */}
      <div className="bg-white rounded-xl shadow-sm p-5 mb-6 border border-gray-100">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 bg-blue-50 text-blue-600 border border-blue-200 px-5 py-3 rounded-lg hover:bg-blue-100 transition-all shadow-sm"
            disabled={refreshing}
          >
            <RefreshCw
      ...
      {/* --- TAB NAVIGATION (DROPDOWN KATEGORI) (GAYA DASHBOARD ADMIN) --- */}
      ...
      {/* --- DAFTAR SURAT (TABLE) (GAYA DASHBOARD ADMIN) --- */}
      <div className="bg-white rounded-xl shadow border border-gray-100 overflow-hidden flex flex-col max-h-[85vh]">
        {/* Header Table (Gaya Dashboard Admin: Judul Kiri, Counter Kanan) */}
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4 flex-shrink-0">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-500 flex-1">
            ...
            <p>
              {searchQuery
                ...
                : "Belum ada surat masuk untuk unit Anda."}
            </p>
          </div>
        ) : (
          // Container Scroll (Gaya Dashboard Admin)
          <div
          ...
          >
            <table className="min-w-full">
              {/* Header Table Sticky (Gaya Dashboard Admin) */}
              ...
              {/* Body Table (Gaya Dashboard Admin: Background Putih Bersih) */}
              <tbody className="divide-y divide-gray-200 text-sm">
                {displayedItems.map((s, index) => {
                  ...
                  return (
                    <tr
                      key={s.id}
                      className={`hover:bg-blue-50 transition-colors duration-200 bg-white`}
                    >
                      <td className="px-6 py-4 whitespace-nowrap text-gray-900 font-mono text-xs border-b border-gray-100">
                        {s.nomor_surat || "-"}
                      </td>
