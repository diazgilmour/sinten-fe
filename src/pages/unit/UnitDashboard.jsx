...
const UnitDashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      // 🔥 LOGIKA UNIT: Hanya ambil surat yang relevan untuk unit ini
      const mySurat = suratData.filter((s) => {
        const myRole = user?.role;
        return (
          s.tujuan === myRole || s.current_unit === myRole || myRole === "admin"
        );
      });

      // --- DATA STATISTIK ---
      // Filter surat yang "Perlu Tindakan" (Pending atau Disposisi/Proses)
      const activeSurat = mySurat.filter((s) => {
  ...
  useEffect(() => {
    ...
    const pendingList = filtered
      ...
      .slice(0, 5); // Batasi 5 item saja seperti Admin
  ...
  const CategoryChart = () => {
    const data = [
      {
        ...
        color: "bg-blue-600",
        icon: <Activity size={20} />, // Menggunakan icon Activity sebagai ganti Briefcase
      },
      {
    ...
    // Cegah maxVal menjadi 0 untuk menghindari pembagian nol
  ...
  if (loading) {
    return (
      <UnitLayout title="Dashboard Unit">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm h-32 animate-pulse" />
            <div className="bg-white p-6 rounded-2xl shadow-sm h-32 animate-pulse" />
          </div>
        </div>
      </UnitLayout>
    ...
    <UnitLayout title="Dashboard Unit">
      <div className="mb-10">
        <p className="text-gray-500 text-base mt-2 leading-relaxed">
          Selamat datang! Ini adalah pusat kendali untuk memproses surat masuk
          yang dialamatkan ke unit Anda.
        </p>
      </div>

      {/* --- ROW 1: STATISTIK UTAMA (HANYA TOTAL & PENDING) --- */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* KOLOM KIRI (SPAN 2): SURAT PERLU TINDAKAN */}
          ...
          <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/30">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Cari..."
                className="pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white w-40 lg:w-56"
              />
            </div>
          </div>
                  ...
                  filteredSuratList.map((s) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <button
                          onClick={() => navigate(`/unit/surat/${s.id}`)}
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
          ...
          {/* Tombol Tambahan (Opsional, Mengisi ruang kosong) */}
          <div
            onClick={() => navigate("/unit/surat")}
            className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-blue-200 transition-all cursor-pointer group flex flex-col justify-center h-full min-h-[120px]"
          >
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900">Daftar Surat</h3>
              <p className="text-sm text-gray-500 mt-1">
                Lihat semua data surat unit
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Tombol Refresh Global (Floating/Corner) */}
      <button
        onClick={() => {
          setRefreshing(true);
          fetchDashboardData();
        }}
        className="fixed bottom-6 right-6 p-3 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-700 transition-all z-50 flex items-center justify-center gap-2"
        disabled={refreshing}
      >
